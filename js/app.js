...
async function search() {
    ...
    try {
        ...
        console.warn('Password protection check failed:', error?.message || error);
        return;
    }
    ...
    try {
        ...
        const resultsArray = await Promise.all(
            selectedAPIs.map(apiId => searchByAPIAndKeyWord(apiId, query))
        );
        ...
        // 排序：标题 > 来源
        allResults.sort((a, b) => {
            const aTitle = (a.norm_title || a.vod_name || '').toString();
            const bTitle = (b.norm_title || b.vod_name || '').toString();
            const nameCompare = aTitle.localeCompare(bTitle, 'zh-Hans-CN');
        ...
        if (counter) counter.textContent = String(allResults.length);
