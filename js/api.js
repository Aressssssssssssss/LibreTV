// ========== 名称+年份 去重工具 ==========
// 标准化标题，去掉常见噪声与符号（不改变业务字段，只用于生成去重 key）
function _normalizeTitleForDedup(name = '') {
    return String(name)
        .replace(/[【】\[\]（）()]/g, ' ')                             // 括号类
        .replace(/\b(4K|1080P|720P|HD|BD|TC|TS|CAM|超清|蓝光|中字|国配|国语|粤语|双语|日语|英语|韩语|导演剪辑|修复版|加长版|抢先|先行|预告|清晰版|无删减)\b/ig, ' ')
        .replace(/[\s·•\.\-_/|]+/g, ' ')
        .trim()
        .toLowerCase();
}
...
function _nameYearKey(item = {}) {
    const rawTitle = item.norm_title || item.vod_name || item.title || '';
    const title = _normalizeTitleForDedup(rawTitle);
    const year = _extractYearForDedup(item); // 可能是空串
    return `${title}__${year}`;
}

// === 跨源去重辅助 ===
// 规范化片名：忽略大小写、空白、常见分隔符，仅保留字母数字（含中日韩）
function normalizeName(name) {
    return (name || '')
        ...
        .replace(/（/g, '(').replace(/）/g, ')')
        .replace(/[·•・\s\-\u3000|｜._，,。:：；;！!？?\[\]【】「」'"]/g, '')
        .replace(/[^\p{L}\p{N}]/gu, ''); // 仅保留字母数字（含 CJK）
}

// 提取年份：优先用字段中的年份；缺失时尝试从片名中抓取
function extractYear(item) {
    const y = (item?.vod_year ?? item?.year ?? '').toString();
    const m1 = y.match(/^(?:19|20)\d{2}$/);
    if (m1) return m1[0];

    const name = (item?.vod_name ?? '').toString();
    const m2 = name.match(/\b(19|20)\d{2}\b/);
    return m2 ? m2[0] : '';
}

// 按“规范化名称+年份”去重；同时把重复来源合并到 alt_sources 里方便后续使用
function dedupeByNameYear(list) {
    const map = new Map();
    for (const item of (list || [])) {
        const key = `${normalizeName(item.vod_name)}_${extractYear(item)}`;
        ...
        if (!map.has(key)) {
            ...
            // 记录其它可用来源（可用于点详情时二选一/兜底）
            kept.alt_sources = kept.alt_sources || [{
                source_name: kept.source_name, source_code: kept.source_code,
                vod_id: kept.vod_id, api_url: kept.api_url
            }];
            kept.alt_sources.push({
                source_name: item.source_name, source_code: item.source_code,
                vod_id: item.vod_id, api_url: item.api_url
            });

            // 简单择优：补齐封面/备注更完整的那条
            if (!kept.vod_pic && item.vod_pic) kept.vod_pic = item.vod_pic;
            if ((!kept.vod_remarks || kept.vod_remarks.length < 2) && item.vod_remarks) {
                kept.vod_remarks = item.vod_remarks;
            }
        }
    }
...
async function handleAggregatedSearch(searchQuery) {
    ...
    try {
        ...
        for (const item of allResults) {
            ...
            if (!k || seenNY.has(k)) continue;
            seenNY.add(k);
            nyDedup.push(item);
        }

        // ② 再按（source+vod_id）做一次兜底去重，防止同源重复
        const uniqueResults = [];
        const seenId = new Set();
        for (const item of nyDedup) {
            const k2 = `${item.source_code}_${item.vod_id}`;
            if (seenId.has(k2)) continue;
            seenId.add(k2);
            uniqueResults.push(item);
        }
...
async function handleMultipleCustomSearch(searchQuery, customApiUrls) {
 ...
 const seenId = new Set();
 for (const item of nyDedup) {
     const k2 = `${item.api_url || ''}_${item.vod_id}`;
     if (seenId.has(k2)) continue;
     seenId.add(k2);
     uniqueResults.push(item);
 }
